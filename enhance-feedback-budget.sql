-- ============================================================
-- ENHANCED FEEDBACK & BUDGET SYSTEM (No Client Portal)
-- PM enters client feedback, tracks revisions, manages budget
-- ============================================================

-- Add revision tracking to client_feedback table
ALTER TABLE client_feedback
ADD COLUMN IF NOT EXISTS revision_count INT DEFAULT 0,
ADD COLUMN IF NOT EXISTS deliverable_file VARCHAR(500) NULL COMMENT 'Link to deliverable file',
ADD COLUMN IF NOT EXISTS client_approval_status ENUM('pending', 'approved', 'revision_requested') DEFAULT 'pending',
...
-- Enhance project_budgets table with billing features
ALTER TABLE project_budgets
ADD COLUMN IF NOT EXISTS billing_type ENUM('fixed', 'hourly', 'mixed') DEFAULT 'fixed',
ADD COLUMN IF NOT EXISTS estimated_hours DECIMAL(10,2) DEFAULT 0,
ADD COLUMN IF NOT EXISTS hourly_rate DECIMAL(10,2) DEFAULT 0 COMMENT 'Project hourly rate',
ADD COLUMN IF NOT EXISTS markup_percentage DECIMAL(5,2) DEFAULT 20 COMMENT 'Profit margin %',
ADD COLUMN IF NOT EXISTS client_quote DECIMAL(10,2) DEFAULT 0 COMMENT 'What client will pay',
ADD COLUMN IF NOT EXISTS internal_cost DECIMAL(10,2) DEFAULT 0 COMMENT 'Our actual costs',
ADD COLUMN IF NOT EXISTS projected_profit DECIMAL(10,2) DEFAULT 0;

-- Add hourly rate to users table (for labor cost calculations)
ALTER TABLE users
ADD COLUMN IF NOT EXISTS hourly_rate DECIMAL(10,2) DEFAULT 50 COMMENT 'Employee hourly rate';
